...
#############################################
    ...
    task1 = PythonOperator(
        ...
        python_callable=run_monitoring,  # 호출할 함수
    ...
    # 다음 DAG를 트리거하는 태스크
    trigger_DAG_Inference = TriggerDagRunOperator(
        task_id='trigger_DAG_Inference',
        trigger_dag_id='DAG_Inference',  # 트리거할 DAG의 ID
        wait_for_completion=False,  # DAG 2의 실행이 끝날 때까지 기다리지 않음
    )    

    #ㅇㅋ
    task1 >> trigger_DAG_Inference
